...
public class EmployeeDao {
    ...
    public static Employee findById(int id) throws SQLException {
        try (Connection c = Database.getConnection();
             PreparedStatement ps = c.prepareStatement(
                 "SELECT employee_id, employee_code, full_name, basic_salary, daily_rate, hourly_rate, is_active FROM Employee WHERE employee_id=?")) {
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next())
                    return new Employee(rs.getInt(1), rs.getString(2), rs.getString(3),
                        rs.getBigDecimal(4), rs.getBigDecimal(5), rs.getBigDecimal(6), rs.getBoolean(7));
            }
        }
        return null;
    }

    public static void insert(String code, String fullName, java.math.BigDecimal basicSalary, java.math.BigDecimal dailyRate, java.math.BigDecimal hourlyRate, boolean active) throws SQLException {
        try (Connection c = Database.getConnection();
             PreparedStatement ps = c.prepareStatement(
                 "INSERT INTO Employee (employee_code, full_name, basic_salary, daily_rate, hourly_rate, is_active) VALUES (?,?,?,?,?,?)")) {
            ps.setString(1, code);
    ...
    public static void update(int id, String code, String fullName, java.math.BigDecimal basicSalary, java.math.BigDecimal dailyRate, java.math.BigDecimal hourlyRate, boolean active) throws SQLException {
        try (Connection c = Database.getConnection();
             PreparedStatement ps = c.prepareStatement(
                 "UPDATE Employee SET employee_code=?, full_name=?, basic_salary=?, daily_rate=?, hourly_rate=?, is_active=? WHERE employee_id=?")) {
            ps.setString(1, code);
            ps.setString(2, fullName);
            ps.setBigDecimal(3, basicSalary);
            ps.setBigDecimal(4, dailyRate);
    ...
    public static class Employee {
        ...
        public final String employeeCode, fullName;
        public final java.math.BigDecimal basicSalary, dailyRate, hourlyRate;
        public final boolean isActive;
        public Employee(int id, String code, String name, java.math.BigDecimal basic, java.math.BigDecimal daily, java.math.BigDecimal hourly, boolean active) {
            employeeId = id; employeeCode = code; fullName = name;
